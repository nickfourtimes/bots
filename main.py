...
BLUESKY_PASSWORD = os.getenv("PODRACING_BSKY_PASSWORD")
...
def extract_prompt(input_string: str) -> str:
    pattern = (
        r"(?i)\bgenerate\s*:\s*(.*)"  # Regex pattern to match variations of "generate:"
    )
    match = re.search(pattern, input_string)
    if match:
        return match.group(
            1
        ).strip()  # Return the matched text, stripping leading and trailing whitespace
    else:
        ...
        if uri.collection == models.ids.AppBskyFeedPost:
            if name_in_text(record["text"]):
                ...
                if posts_in_thread.thread.parent == None:
                    ...
                    entries.extend(
                        parse_thread(
                            posts_in_thread.thread.post.author.did,
                            posts_in_thread.thread.parent,
                        )
                    )
                ...
                response_text.text(
                    f"Hey, @{poster_profile.handle}. Here's a image generated from your post. "
                )
...
# client.send_post("hey there it's me")
